<script lang="ts">
	...
	interface Props {
		...
		/** 여러 cycle의 metrics 배열 (loop 있을 때) — [{cycle: 1, metrics: {...}}, ...] */
		cycleMetrics?: { cycle: number; metrics: Record<string, number> }[];
	}

	let { metrics, cycleMetrics = [] }: Props = $props();

	// ── prefix 제거 (시나리오에서 r{N}_step{N}_ prefix가 붙음) ──
	function stripPrefix(key: string): string {
		return key.replace(/^r\d+_(loop\d+_)?step\d+_/, '');
	}

	// prefix 제거된 metrics
	const cleanMetrics = $derived.by(() => {
		const result: Record<string, number> = {};
		for (const [k, v] of Object.entries(metrics)) {
			result[stripPrefix(k)] = v;
		}
		return result;
	});

	// ── 매크로 결과 감지 ──
	const isMacroResult = $derived(
		Object.keys(cleanMetrics).some(k => k.endsWith('_score') || k.endsWith('_speed_mbs'))
	);
	...
	const scoreEntries = $derived.by<MetricEntry[]>(() => {
		return Object.entries(cleanMetrics)
	...
	const versionEntry = $derived.by(() => {
		const entry = Object.entries(cleanMetrics).find(([k]) =>
	...
	const cleanCycleMetrics = $derived.by(() =>
		cycleMetrics.map(cm => ({
			cycle: cm.cycle,
			metrics: Object.fromEntries(Object.entries(cm.metrics).map(([k, v]) => [stripPrefix(k), v]))
		}))
	);

	const hasLoop = $derived(cleanCycleMetrics.length > 1);
	...
	const loopScoreKeys = $derived.by(() => {
		...
		for (const cm of cleanCycleMetrics) {
	...
	function buildLoopChart(key: string) {
		const data = cleanCycleMetrics.map(cm => ({ cycle: cm.cycle, value: cm.metrics[key] ?? 0 }));
	...
	const scoreTableData = $derived.by<TableRow[]>(() => {
		if (hasLoop) {
			return cleanCycleMetrics.map(cm => {
